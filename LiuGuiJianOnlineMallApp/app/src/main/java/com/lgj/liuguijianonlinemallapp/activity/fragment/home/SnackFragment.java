...
public class SnackFragment extends Fragment {
    private RecyclerView rv_snack;
    private List<Goods> goods=new ArrayList();
    private MyGoodsRecyclerViewAdapter adapter;
    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        final View view = inflater.inflate(R.layout.module_fragment_snack, container, false);
        init(view);
        loadData();
        return view;
    }
    private void init(View view){
        rv_snack=view.findViewById(R.id.rv_snack);
        adapter=new MyGoodsRecyclerViewAdapter(getContext(),goods);
        rv_snack.setLayoutManager(new StaggeredGridLayoutManager(2, StaggeredGridLayoutManager.VERTICAL));
        rv_snack.setAdapter(adapter);
