...
public class ManFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        final View view = inflater.inflate(R.layout.module_fragment_man, container, false);
        init(view);
        loadData();
        return view;
    }
    private void init(View view){
        rv_man=view.findViewById(R.id.rv_man);
        adapter=new MyGoodsRecyclerViewAdapter(getContext(),goods);
        rv_man.setLayoutManager(new StaggeredGridLayoutManager(2, StaggeredGridLayoutManager.VERTICAL));
